...
TUPLE_OF_KEYS = ('id', 'name', 'created_at', 'massage')
SELECT_ALL_MASSAGES = """SELECT * FROM massage"""
SELECT_YOUR_MASSAGE = """SELECT * FROM massage WHERE m_created_at = %s"""
SELECT_ONE_MASSAGE = """SELECT * FROM massage WHERE m_id = %s"""
INSERT_MASSAGE = """INSERT INTO massage (m_name, m_created_at, m_massage) VALUES (%s, %s, %s)"""
XML_SUFFIX = 'xml'
JSON_SUFFIX = 'json'
GUESTBOOK = 'guestbook'
SIZE = 'size'
...
GUESTBOOK_MASSAGE = 'guestbook-message'
TIME_FORMAT = "%Y-%m-%d %H:%M:%S"
PATH_TO_MASSAGE_FROM_POST = '/guestbook/guestbook-message'
...
def data_to_xml(data, all_book):
    ...
    for i in range(len(data)):
        child = etree.SubElement(root, GUESTBOOK_MASSAGE, attrib={ID: str(data[i][0]), NAME: data[i][1],
                                                                  CREATED_AT: data[i][2]})
        child.text = data[i][3]
...
def get_data_from_post(body, format_):
    ...
    if format_ == JSON_SUFFIX:
        ...
        tree = etree.XML(data)
        notes = tree.xpath(PATH_TO_MASSAGE_FROM_POST)
        name = notes[0].get(NAME)
        massage = notes[0].text
    insert_data = (name, created_at, massage)
    return insert_data


def making_insert_to_db(insert_data):
    get_data(INSERT_MASSAGE, insert_data, False)
...
class MainHandler(tornado.web.RequestHandler):
    ...
    def get(self):
        rows = get_data(SELECT_ALL_MASSAGES, None)
    ...
    def post(self):
        ...
        rows = get_data(SELECT_YOUR_MASSAGE, (condition,))
...
class MessageHandler(tornado.web.RequestHandler):
    ...
    def get(self, massage_id):
        rows = get_data(SELECT_ONE_MASSAGE, (massage_id,))
        data = mapping_depending_on_the_suffix(rows, self.format_, False)
        self.write(data)
